...
def plot_vertical_section(
    ...
    lineStyle : str, optional
        the line style of contour lines (if specified); this applies to the
        style of contour lines of ``field`` (the style of the contour lines
        of ``contourComparisonField`` is set using
        contourComparisonLineStyle).

    lineColor : str, optional
        the color of contour lines (if specified); this applies to the
        contour lines of ``field`` (the color of the contour lines of
    ...
    contourComparisonField : xarray.DataArray, optional
    ...
    originalFieldName : str, optional
       the name for ``field`` (for the purposes of labeling the
    ...
    if movingAvgPoints is not None and movingAvgPoints != 1:
        field = field.rolling(dim={field.dims[0]: movingAvgPoints}, center=True).\
            mean().dropna(field.dims[0])
        x = field.rolling(dim={x.dims[0]: movingAvgPoints}, center=True).\
            mean().dropna(x.dims[0])
        y = field.rolling(dim={y.dims[0]: movingAvgPoints}, center=True).\
            mean().dropna(y.dims[0])

    colormapDict = setup_colormap(config, colorMapSectionName, suffix=suffix)

    if not plotAsContours:
        # display a heatmap of field
    ...
    landArray = np.ma.where(field != np.nan, 1.0, field)
