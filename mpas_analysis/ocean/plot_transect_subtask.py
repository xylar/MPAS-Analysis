...
class PlotTransectSubtask(AnalysisTask):
    ...
    def _get_contour_colormaps():
        ...
        # passes through 0, 1/2 and 1 but slowly at the ends and fast in the
        # middle.  The blue-greens in the center of the colormap are hard to
        # distinguish
        cubic = -2*x**3 + 3*x**2
        main_cmap = colors.LinearSegmentedColormap.from_list(
            f'trunc_{cmap.name}', cmap(cubic))
